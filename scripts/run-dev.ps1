...
try {
    ...
    # Loopback only. A LAN address in this list would silently defeat the very
    # guard the block exists to enforce, so none is whitelisted.
    ...
    # NOTE: 'alembic upgrade head' cannot build an EMPTY database in this repo.
    # The first revision (97472310f3a2 "Initial migration") was autogenerated
    # against an already-populated SQLite DB and opens with
    # 'ALTER TABLE projects ...' on a table nothing ever creates -- the
    # CLAUDE.md rule 8 violation. On a brand new dev database, bootstrap the
    # schema once with scripts/bootstrap_dev_schema.py (ORM metadata + stamp),
    # after which this step is the normal incremental upgrade.
    if (-not $SkipMigrations) {
        Write-Host "`nApplying migrations..." -ForegroundColor Cyan
        & $venvPython -m alembic upgrade head
        if ($LASTEXITCODE -ne 0) {
            Write-Fail "alembic upgrade head failed."
            Write-Info "On a freshly created dev database this is expected: the migration"
            Write-Info "chain cannot build an empty schema. Run this once, then retry:"
            Write-Info "  $venvPython scripts\bootstrap_dev_schema.py"
            exit 1
        }
    ...
    if (-not $NoReload) {
        ...
        # 'app' and 'ml' are watched too, and '.' is NOT: the root holds the
        # facades (main.py, models.py, schemas.py, config.py, database.py) but
        # also uploads/ and logs/ when DATA_DIR is the tree, so the root modules
        # are picked up via --reload-include instead of watching everything.
        $uvicornArgs += @(
            "--reload",
            "--reload-dir", "api",
            "--reload-dir", "app",
