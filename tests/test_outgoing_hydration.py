"""Runs the outgoing-task hydration spec under node, from pytest.

Same arrangement as `test_draft_project_scope.py`: the frontend has no build
step and no JS test runner, so the behaviour spec is a plain node script and
this shim surfaces it to pytest.
...
SPEC = Path(__file__).parent / "js" / "outgoing_hydration_spec.mjs"


@pytest.mark.skipif(shutil.which("node") is None, reason="node not installed")
def test_outgoing_hydration_spec():
