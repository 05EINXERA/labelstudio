"""Runs the client wipe-guard spec under node, from pytest.

Same arrangement as `test_outgoing_hydration.py`: the frontend has no build
step and no JS test runner, so the behaviour spec is a plain node script and
this shim surfaces it to pytest. The server half is tests/test_bulk_loss_guard.py.
...
SPEC = Path(__file__).parent / "js" / "wipe_guard_spec.mjs"


@pytest.mark.skipif(shutil.which("node") is None, reason="node not installed")
def test_wipe_guard_spec():
