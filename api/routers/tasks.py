...
from schemas import BulkDelete, BulkUpdate, TaskUpdate
    ...
    if task.id:
        ...
        if db_task:
            if task.updated_at and db_task.updated_at:
                try:
                    client_updated = datetime.datetime.fromisoformat(
...
def bulk_delete_tasks(payload: BulkDelete, db: Session = Depends(get_db)):
    ...
    db.query(models.Task).filter(models.Task.id.in_(payload.ids)).delete(
...
def bulk_update_tasks(payload: BulkUpdate, db: Session = Depends(get_db)):
    ...
    if update_data:
        ...
        db.query(models.Task).filter(models.Task.id.in_(payload.ids)).update(
